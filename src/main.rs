...
// return Err(anyhow::anyhow!("No valid pricipals found after filtering.").into());
    ...
    let principals: Vec<String> = vec![short_name.clone()];
    if principals.is_empty() {
        error!(
            "No valid principals due to empty 'short_name'",
        );
        return Err(anyhow::anyhow!("No valid pricipals due to empty 'short_name'.").into());
    }
